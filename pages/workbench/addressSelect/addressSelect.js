...
VM.confirmSelect = function() {
    let data = this.data
    let valiate = data.provinceIndex >= 0 && data.provinceIndex >= 0 && data.areaIndex >= 0
    if (!valiate) {
        return util.Toast('请选择完整地址信息')
    }
    ...
    prevPage.setData({
        address: address,
        // 省市区下标
        provinceId: data.provinceList[data.provinceIndex].id,
        cityId: data.cityList[data.cityIndex].id,
        areaId: data.areaList[data.areaIndex].id
    });
    //返回上一页
    wx.navigateBack({
        delta: 1
